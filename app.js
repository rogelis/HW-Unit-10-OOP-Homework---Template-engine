...
function buildHtmlPage() {
    ...
    fs.writeFileSync("index.html", newFile, function (err) {
        if (err) throw err;
    })
    ...
    fs.appendFileSync("index.html", "</div></main></body></html>", function (err) {
        if (err) throw err;
    });
...
function buildHtmlCard(memberType, name, id, email, propertyValue) {
    ...
    fs.appendFileSync("index.html", data, err => { if (err) throw err; })
